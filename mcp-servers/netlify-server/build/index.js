...
const isValidArgs = (args) => typeof args === 'object' && args !== null;
class NetlifyServer {
    server;
    constructor() {
    ...
    async apiRequest(endpoint, options = {}) {
        ...
        if (!response.ok) {
            ...
            throw new McpError(ErrorCode.InternalError, `Netlify API error: ${response.status} - ${error}`);
        }
                ...
                switch (request.params.name) {
                    case 'list_sites': {
                        ...
                        const filtered = args.filter
                            ? sites.filter((s) => s.name.includes(args.filter))
                            : sites;
                        return {
                    ...
                    case 'get_site': {
                        ...
                        const site = await this.apiRequest(`/sites/${site_id}`);
                        return {
                    ...
                    case 'list_deploys': {
                        ...
                        const deploys = await this.apiRequest(`/sites/${site_id}/deploys?per_page=${per_page}`);
                        return {
                    ...
                    case 'get_deploy': {
                        ...
                        const deploy = await this.apiRequest(`/deploys/${deploy_id}`);
                        return {
                    ...
                    case 'create_deploy': {
                        ...
                        const deploy = await this.apiRequest(`/sites/${site_id}/deploys`, {
                            method: 'POST',
                    ...
                    case 'list_forms': {
                        ...
                        const site_id = request.params.arguments.site_id;
                        const forms = await this.apiRequest(`/sites/${site_id}/forms`);
                        return {
                    ...
                    case 'list_form_submissions': {
                        ...
                        const submissions = await this.apiRequest(`/forms/${form_id}/submissions`);
                        return {
                    ...
                    case 'list_functions': {
                        ...
                        const site_id = request.params.arguments.site_id;
                        const functions = await this.apiRequest(`/sites/${site_id}/functions`);
                        return {
