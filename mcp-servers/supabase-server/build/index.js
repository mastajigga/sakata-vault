...
class SupabaseServer {
    ...
    async apiRequest(endpoint, options = {}) {
        ...
        if (!response.ok) {
            ...
            throw new McpError(ErrorCode.InternalError, `Supabase API error: ${response.status} - ${error}`);
        }
    ...
    buildQueryString(filters, select, limit, offset, order, orderDesc) {
            ...
            try {
                switch (request.params.name) {
                    case 'query_table': {
                        ...
                        const query = this.buildQueryString(args.filters, args.select, args.limit, args.offset, args.order, args.order_desc);
                    ...
                    case 'list_tables': {
                        const data = await this.apiRequest('/rpc/get_tables?select=table_name,table_schema&table_schema=eq.public');
                        return {
                            content: [{ type: 'text', text: JSON.stringify(data, null, 2) }],
                        };
                    }
                    case 'get_table_schema': {
                        ...
                        const data = await this.apiRequest(`/rpc/get_table_columns?select=column_name,data_type,is_nullable&table_name=eq.${table}`);
                        return {
                            content: [{ type: 'text', text: JSON.stringify(data, null, 2) }],
                        };
                    }
