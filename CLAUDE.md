...
- **Traffic Control & Instrumentation (Proxy)** : Le client Supabase dans `src/lib/supabase.ts` est enveloppé dans un Proxy évolué qui gère une **file d'attente de requêtes** :
  - **Limite de Concurrence** : Maximum 4 requêtes Postgrest simultanées pour garantir 2 sockets libres pour l'Auth et les assets.
  - **Priorisation** : Les requêtes vers la table `profiles` passent en tête de file (critique pour l'UI).
  - **Logs** : `[NET-QUEUE-WAIT]`, `[NET-QUEUE-EXEC]`, `[NET-START]`, `[NET-RESOLVED]`, `[NET-SATURATION]`.
- **AbortController OBLIGATOIRE** : Tout `useEffect` lançant une requête Supabase **DOIT** utiliser un `AbortController` et appeler `controller.abort()` dans la fonction de nettoyage pour libérer les sockets et la file d'attente immédiatement.
...
| Date | Modification |
|------|-------------|
| 2026-05-09 | **NAV MOBILE CONSTELLATION + correctifs v3.5.1** — Refonte navigation mobile : FAB doré bottom-right qui s'épanouit en arc avec 2 batches (Essentiel ↔ Découverte) via bouton-bascule. Action contextuelle dorée selon route (Nouveau sujet, Répondre, Ajouter membre…). Hide on scroll-down. Portail React pour les satellites + backdrop blur plein écran. Hand-tuned ANGLE_SEQUENCES pour éviter chevauchement de labels. Hamburger legacy caché. Fixes : bouton "Devenir contributeur" sur home (route + redirect post-login), modale signalement portée plein-écran, page /notifications créée (404 corrigé), forum thread/categorie ambiguous join réparé, chat auto-scroll en bas à l'ouverture. **Changelog admin** : entrées v3.2 → v3.5 ajoutées dans `src/data/changelog.json` (rattrapage des 4 versions manquantes au Notification Center). |
| 2026-05-09 | **ABONNEMENTS PREMIUM v3.5.0** — Tiers d'articles : `article_type ∈ {summary, poetic, philosophical}`. Résumés libres pour tous (anon inclus). Poétique + philosophique réservés aux abonnés Premium/Elite. Admins/managers peuvent offrir un abonnement gratuit (7j/30j/90j/1an/illimité) via Crown button sur `/admin/users` → modale `GrantSubscriptionModal`. Table `subscription_grants` (audit + ack). Composant global `SubscriptionGrantGate` affiche modale festive avec couronne/sparkles + décompte au prochain login, doublé d'une notification dans la cloche (`subscription_granted` / `subscription_revoked`). API `/api/admin/subscriptions/grant` (POST + DELETE). Page `/admin/article/[id]/review` propose désormais un sélecteur Résumé / Poétique / Philosophique au moment de la publication. |
