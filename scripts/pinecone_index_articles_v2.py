#!/usr/bin/env python3
"""
Pinecone Article Indexer v2 — Optimized for batch indexing

This version loads the model ONCE and indexes all articles efficiently.
Much faster than v1 which reloaded the model for each article.

CRITICAL RULE: Index ONLY the .content field (books), NEVER .summary (articles)

Usage:
    python scripts/pinecone_index_articles_v2.py [--dry-run] [--namespace iluo_livres_site]
"""
...
try:
    ...
    print("⚠️  Note: Pinecone or SentenceTransformers not available. Running in preview mode.")
...
class PineconeArticleIndexer:
    """Optimized indexer that loads model once"""
    ...
    def __init__(self, api_key: str, index_name: str = "sakata", dry_run: bool = False):
        ...
        # Initialize Pinecone and model once
        self.index = None
        self.model = None

        if not dry_run and PINECONE_AVAILABLE:
            self._init_pinecone()
    ...
    def index_article(self, article: Dict[str, Any], namespace: str) -> bool:
        ...
        if self.dry_run:
            ...
            try:
                # Create embedding
                embedding = self.model.encode(f"passage: {content_fr}").tolist()

                # Store metadata (first 1000 chars of content)
                metadata["text"] = content_fr[:1000]
...
def main():
    parser = argparse.ArgumentParser(
        description="Index Kisakata articles into Pinecone (optimized v2)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    # Parse articles
    parser_ts = TypeScriptArticleParser(str(articles_file))
    ...
    print(f"\n📊 Namespace Structure:")
    print(f"   iluo_livres_academiques  → Academic PDFs (existing)")
