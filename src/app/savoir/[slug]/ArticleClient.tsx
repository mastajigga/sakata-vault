...
const ArticleClient: React.FC<ArticleClientProps> = ({ initialArticle }) => {
  ...
  useEffect(() => {
    // If we already have initial data, skip loading state but still fetch fresh data
    if (initialArticle) {
      setLoading(false);
    }
  ...
  const renderHtml = (text: string) => {
    return text
      .replace(
        /^# (.*)/gm,
        '<h1 class="font-display text-4xl md:text-5xl font-bold mt-12 mb-8 text-ivoire-ancien">$1</h1>'
      )
      .replace(
        /^## (.*)/gm,
        '<h2 class="font-display text-2xl md:text-3xl font-bold mt-16 mb-6 text-ivoire-ancien border-b border-white/5 pb-4">$1</h2>'
      )
      .replace(
        /^### (.*)/gm,
        '<h3 class="font-display text-xl md:text-2xl font-bold mt-12 mb-4 text-ivoire-ancien/90">$1</h3>'
      )
      ...
      switch (block.type) {
        case "text":
          return (
            <div
              key={block.id || i}
              className="mb-0"
              dangerouslySetInnerHTML={{ __html: renderHtml(block.body || "") }}
            />
          );
        case "heading":
          return (
            <h2
              key={block.id || i}
              className="font-display text-2xl md:text-3xl font-bold mt-12 mb-6 text-ivoire-ancien border-b border-white/5 pb-4"
            >
              {block.body}
            </h2>
          );
        case "image":
          const alignmentClasses =
            block.alignment === "left"
              ? "float-left md:-ml-24 mr-8 mb-8 w-full md:w-[60%]"
              : block.alignment === "right"
                ? "float-right md:-mr-24 ml-8 mb-8 w-full md:w-[60%]"
                : "w-full mb-12";
          return (
            <div
              key={block.id || i}
              className={`${alignmentClasses} clear-both group`}
            >
              ...
              {block.caption && (
                <p className="mt-4 text-xs italic opacity-40 font-light px-4">
                  {block.caption}
                </p>
              )}
            </div>
        ...
        case "quote":
          return (
            <blockquote
              key={block.id || i}
              className="pl-8 border-l-2 border-or-ancestral/50 my-16 py-4 bg-or-ancestral/[0.03] pr-8 rounded-r-3xl"
            >
              <p className="text-2xl md:text-3xl font-display italic text-ivoire-ancien/90 mb-6 leading-relaxed">
                &ldquo;{block.body}&rdquo;
              </p>
              {block.caption && (
                <cite className="text-sm font-bold text-or-ancestral not-italic tracking-widest uppercase">
                  — {block.caption}
                </cite>
              )}
            </blockquote>
      ...
      <section className="relative h-[65vh] md:h-[75vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 bg-[var(--foret-nocturne)]">
          {article.hero_video_url ? (
            ...
            >
              <source
                src={article.video_background || "/videos/wan-iluo-into-the-eyes.mp4"}
                type="video/mp4"
              />
            </video>
          )}
          ...
          <div className="lg:col-span-8">
            ...
            >
              {(() => {
                ...
                return (
                  <div dangerouslySetInnerHTML={{ __html: renderHtml(mainBody as string) }} />
                );
              })()}
            </div>
            ...
            <div className="sticky top-32 space-y-8">
              ...
              <div className="p-8 bg-[var(--eau-sombre)] rounded-[2.5rem] border border-white/5 shadow-xl">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xl font-mono text-ivoire-ancien">
                ...
                <div className="mt-8 pt-8 border-t border-white/5">
                  <p className="text-[10px] uppercase tracking-[0.2em] font-bold mb-4 text-or-ancestral">
                    Contexte Culturel
                  </p>
              ...
              >
                <div className="flex items-center justify-between px-6 py-4">
                  <span className="text-xs font-bold text-or-ancestral uppercase tracking-widest">
                    Rejoindre la palabre
                  </span>
                </div>
              </Link>
            </div>
